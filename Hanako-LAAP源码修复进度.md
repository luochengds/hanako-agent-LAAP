...
当前为第一阶段硬门控：默认使用 `AGIAgent`，`LaapBridge` 保留为显式兼容 fallback；Codex/Lifelike 兼容分支也已通过 `wrap_runtime_agent()` 接入。Hanako Desktop 的 `prompt()` / `promptSession()` 已增加 Sidecar gate，设置 `LAAP_PSI_GATE_REQUIRED=1` 后进入严格模式。真实 Sidecar 联动探针已通过：`/before_turn`、`/after_turn` 均返回 HTTP 200。`CognitiveRuntime` 统一事务接口已建立；`AGIAgent.process_interaction` 已成为默认认知管线，设置 `LAAP_COGNITIVE_RUNTIME=bridge` 可显式回退。Bridge/AGIAgent 离线比较工具已建立，默认 fixture 运行结果为两者各 6/6 成功；第二版已加入 needs/attention 首末状态差异比较，AGIAgent 的需求和注意力变化可被观测；第三版已加入隔离 state_dir 重载验证；初次发现世界模型/CognitiveBus 摘要恢复不完整，已修复 AGIAgent CognitiveBus 与 AbstractWorldModel 持久化。修复后 AGIAgent 6 次 interaction 可恢复，stable_summary_matches=True。随后已完成 PSIDriver canonical 接管：AGIAgent → PSIDriver → AGIAgent core，补齐 Act/response、bypass 语义、重入锁、注意力单源、learn 兼容钩子和异步 Consciousness Harness 桥接；canonical/shadow 各 6/6，核心回归 108 passed。DeepSeek 真实 LLM Act 已通过（channel_configured=True、used_fallback=False、error=None），Desktop/Sidecar E2E 也已通过。
