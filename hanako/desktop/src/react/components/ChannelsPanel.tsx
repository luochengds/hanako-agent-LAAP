...
export function ChannelInput() {
  ...
  const handleAttach = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      for (const file of files) {
        ...
        if (upload?.dest) {
          ...
          const markdown = isImage
            ? `![${upload.name || file.name}](${upload.dest})`
            : `[${upload.name || file.name}](${upload.dest})`;
          setInputValue(prev => (prev ? prev + '\n' + markdown : markdown));
        }
      }
    } catch (err) {
      console.warn('[channel] attach upload error:', err);
    } finally {
      setUploading(false);
    }
  }, [uploading, readFileAsBase64]);
  ...
  return (
    <div className={`${styles.channelInputWrapper}${expanded ? ` ${styles.channelInputWrapperExpanded}` : ''}`}>
      ...
      <div className={styles.channelInputRow}>
        <button
          type="button"
          className={styles.channelExpandBtn}
        ...
        <textarea
          ref={inputRef}
          className={`${styles.channelInputBox}${expanded ? ` ${styles.channelInputBoxExpanded}` : ''}`}
