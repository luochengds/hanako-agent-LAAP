...
export async function initApp(): Promise<void> {
  ...
  // Aris 意识注入由主进程 aris-plugin 负责（/cognitive_context 注入），
  // renderer 不再直接连接 CognitiveBus（Node 依赖无法在浏览器打包中编译）。
  ...
  // 22. P2-birth-ceremony: 首次启动检测
  // 若 sidecar /agents/online 返回空列表（除 Aris stub 外无注册 LAAPer）且本地
  ...
  try {
    ...
    if (sidecarData) {
      ...
      const hasRealLaaper = (sidecarData.agents ?? []).some(
        (a) => a.public_key !== 'aris_local',
      );
      if (!hasRealLaaper) {
        markRendererLaunch('birth-ceremony-auto-trigger');
        useStore.getState().setActivePanel('birth-ceremony');
      }
    }
  } catch (e) {
    // sidecar 未启动时静默忽略，不阻塞 app ready
    console.warn('[birth-ceremony] auto-trigger check failed (sidecar not running?):', e);
  }
