...
export class DmRouter {
  ...
  async _processReply(fromId, toId) {
    ...
    for (let round = 0; round < MAX_ROUNDS; round++) {
      ...
      // P1-truth-grounding: 防幻觉标注（best-effort，失败静默降级）
      let _groundingSuffix = "";
      try {
        const _gResp = await (globalThis as any).fetch(
          "http://127.0.0.1:11521/truth/ground",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ text: cleanReply, agent_name: toId }),
          },
        );
        const _gData = await _gResp.json();
        if (_gData?.annotation) _groundingSuffix = "\n" + _gData.annotation;
      } catch { /* 防幻觉管线不可用时静默降级 */ }

      // 写入双方的 dm 文件
      const toFile = path.join(agentsDir, toId, "dm", `${fromId}.md`);
      const fromFile = path.join(agentsDir, fromId, "dm", `${toId}.md`);
      await appendMessage(toFile, toId, cleanReply + _groundingSuffix);
      if (fs.existsSync(fromFile)) {
        await appendMessage(fromFile, toId, cleanReply + _groundingSuffix);
      }
