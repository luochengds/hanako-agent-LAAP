...
export class AgentManager {
  ...
  async createAgent({ name, id, yuan, enabledSkills, initialFiles, avatarPath, initialMemory }) {
    ...
    // P1-memory-vault: 触发 sidecar 初始化 agent vault（best-effort，不阻塞创建）
    // 调用 POST http://127.0.0.1:{sidecarPort}/vault/init?agent={agentId}
    // 若 sidecar 未运行或调用失败，仅记录警告，agent 创建照常成功
    this._triggerVaultInit(agentId).catch((err) => {
      log.warn(`vault init for ${agentId} failed (non-blocking): ${err?.message || err}`);
    });
  ...
  /**
   * P1-memory-vault: 调用 sidecar POST /vault/init 初始化 agent 的独立加密 vault。
   * 幂等：sidecar 端 VaultManager.init_for_agent 重复调用不会破坏已有数据。
   * Best-effort：sidecar 不可达时静默失败，不阻塞 agent 创建流程。
   */
  async _triggerVaultInit(agentId) {
    const port = this._d?.arisSidecarPort || 11521;
    const http = await import("http");
    return new Promise((resolve, reject) => {
      const body = JSON.stringify({ agent_name: agentId });
          ...
          res.on("end", () => {
            try {
              ...
              if (parsed && parsed.initialized) {
                log.log(`vault initialized for ${agentId} at ${parsed.vault_path}`);
              }
              resolve(parsed);
            } catch (e) {
              resolve({ raw: data });
