...
import { buildConversationMarkdownExport, buildConversationHtmlExport, buildConversationDocxExport } from "../../lib/channels/conversation-export.ts";
  ...
  route.get("/conversations/:id/export", async (c) => {
    try {
      ...
      const filters = {
        ...
        speakers: c.req.query("speaker") ? c.req.query("speaker")!.split(",").map((s: string) => s.trim()).filter(Boolean) : undefined,
      };
      // Remove undefined keys
      Object.keys(filters).forEach((k) => { if (!(filters as any)[k]) delete (filters as any)[k]; });
      let buildExport;
      if (format === "html") buildExport = buildConversationHtmlExport;
      else if (format === "docx") buildExport = buildConversationDocxExport;
      else buildExport = buildConversationMarkdownExport;
      let archive;
      if (id.startsWith("dm:")) {
      ...
      console.error(`[EXPORT ERROR] /conversations/${id}/export:`, error);
