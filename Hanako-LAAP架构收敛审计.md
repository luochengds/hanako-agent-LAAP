...
新增 `laap/runtime/psi_gateway.py`。canonical API/CLI Agent 的 `run()`、`chat()`、`chat_stream()` 现在经过强制 PSI before/after 边界；PSI 初始化或边界失败时不静默绕过。Codex/Lifelike 兼容分支也通过 `wrap_runtime_agent()` 接入同一边界。Hanako Desktop 的 `prompt()` / `promptSession()` 已增加 Sidecar PSI gate，使用 `LAAP_PSI_GATE_REQUIRED=1` 开启严格模式。当前默认使用仓库内 `LaapBridge`，若完整 `laap_brain` 不可用会使用其 fallback cognitive kernel，因此这是第一阶段硬门控，不等同于完整 `laap.agi.PSIDriver` 六阶段接管。
